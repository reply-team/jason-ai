...
/// <summary>
/// The global <c>--actor &lt;type&gt;[:&lt;id&gt;]</c> claim, so a skill running as a role journals under that role.
/// The CLI only parses the text: claims are not verified anywhere, and the one rule enforced here is that
/// <c>system</c> belongs to the runtime's own writes and cannot be claimed from outside.
/// </summary>
public static class ActorOption
{
    ...
    {
        Description = "Who performs the operation: human, human:<id> or role:<id>.",
        Recursive = true,
    };
    ...
    public static JsonObject? Parse(string? text)
    {
        ...
        if (type is not "human" and not "role")
        {
            throw new UsageException($"{Name} must be 'human', 'human:<id>' or 'role:<id>'.");
        }
