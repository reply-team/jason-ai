...
public partial class InteractiveSkillTests
{
    ...
    public void The_operating_skill_says_what_a_session_cannot_do_and_what_is_not_released_yet()
    {
        ...
        // On Windows the two autostart verbs are refused outright from a medium-integrity process. A skill
        // that did not name the elevated prompt would have a session report the Task Scheduler's "Access is
        // denied" as a broken installation, and a person would go looking for a fault that is not there.
        ...
        // The other half of the same fact. A session that believed every autostart verb was out of reach
